...
const props = defineProps({
    candidates: Object, branches: Array, filters: Object,
    statusOptions: Object, finalStatusOptions: Object, profileOptions: Object, applicantTypeOptions: Object
})
...
watch([search, branchId, status, finalStatus, profile, applicantType], () => {
    router.get(route('admin.candidates.index'), {
        search: search.value, branch_id: branchId.value,
        status: status.value, final_status: finalStatus.value, profile: profile.value, applicant_type: applicantType.value
        ...
        <div class="space-y-5">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex flex-wrap gap-3">
                ...
                <select v-model="profile" class="input-field w-44">
                    ...
                    <option v-for="(l, k) in profileOptions" :key="k" :value="k">{{ l }}</option>
                </select>
                ...
                <select v-model="finalStatus" class="input-field w-40">
                    ...
                    <option v-for="(l, k) in finalStatusOptions" :key="k" :value="k">{{ l }}</option>
                </select>
                <select v-model="applicantType" class="input-field w-56">
                    <option value="">All Applicant Types</option>
                    <option v-for="(l, k) in applicantTypeOptions" :key="k" :value="k">{{ l }}</option>
                </select>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 border-b border-gray-100">
                        <tr>
                            ...
                            <th class="px-5 py-3 text-left font-medium text-gray-500">Position / Profile</th>
                            ...
                            <th class="px-5 py-3 text-left font-medium text-gray-500">Final</th>
                            <th class="px-5 py-3 text-left font-medium text-gray-500">Round</th>
                            <th class="px-5 py-3 text-left font-medium text-gray-500">Registered</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        <tr v-for="c in candidates.data" :key="c.id" class="hover:bg-gray-50">
                            ...
                            <td class="px-5 py-4">
                                <p class="text-gray-700">{{ c.position_applied }}</p>
                                <p class="text-xs text-gray-400">{{ c.profile_category }}</p>
                            </td>
                            ...
                            <td class="px-5 py-4">
                                <StatusBadge :status="c.final_status" type="final" />
                            </td>
                            <td class="px-5 py-4 text-gray-500 text-xs">{{ c.current_round?.name ?? '—' }}</td>
                            <td class="px-5 py-4 text-gray-400 text-xs">{{ c.registration_date?.split('T')[0] }}</td>
                            <td class="px-5 py-4">
                        ...
                        <tr v-if="!candidates.data?.length">
                            <td colspan="8" class="px-5 py-10 text-center text-gray-400">No candidates found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
